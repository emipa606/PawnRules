...
{
    /// <summary>
    ///     Called when the button for this option is clicked. Setting the value must be handled by the delegate. Unused if
    ///     this option does not implement a button widget.
    /// </summary>
    ...
    /// <summary>
    ///     Called when the button for this option is clicked in the default rules dialog. Setting the value must be handled by
    ///     the delegate. Unused if this option does not implement a button widget.
    /// </summary>
    ...
    internal OptionHandle(AddonOption addon)
    {
        Addon = addon;
    }
    ...
    /// <summary>
    ///     Called when the button for this option is clicked in the default rules dialog. Unused if this option does not
    ...
    {
        if (this is not OptionHandle<T> handle)
        {
            throw new Mod.Exception($"Invalid OptionHandle for type {typeof(T).Name}");
        }

        handle.SetValue(pawn,
            handle.OnChangeForPawnForPawn == null
                ? newValue
                : handle.OnChangeForPawnForPawn(pawn, handle.GetValue(pawn), newValue));
    }
    ...
    internal void ChangePresetValue<T>(Rules rules, T newValue)
    {
        ...
        rules.SetAddonValueDirect(handle.Addon,
            handle.OnChangeForForPreset == null
                ? newValue
                : handle.OnChangeForForPreset(rules.Type.AsTarget,
                    rules.GetAddonValue(handle.Addon, (T)handle.Addon.DefaultValue), newValue));
    }

    internal void DoClick(Pawn pawn)
    {
        OnButtonClick(pawn);
    }

    internal void DoDefaultClick(OptionTarget target)
    {
        OnDefaultButtonClick(target);
    }
}

/// <summary>
///     Provides a handle for a rules option. This class may not be instantiated manually, create a
///     <see cref="PawnRulesLink" /> to add one.
/// </summary>
...
public class OptionHandle<T> : OptionHandle
{
    ...
    /// <summary>
    ///     This is called when the value of the option is changing for a preset type. Unused if the option implements a
    ///     button.
    /// </summary>
    ...
    /// <param name="invalidValue">The value returned if unable to retrieve the option.</param>
    /// <returns>Returns the value if the option is found or <see paramref="invalidValue" /> if not.</returns>
    public T GetDefaultValue(OptionTarget target, T invalidValue = default)
    {
        return Registry.GetAddonDefaultValue<T>(target, Addon);
    }
