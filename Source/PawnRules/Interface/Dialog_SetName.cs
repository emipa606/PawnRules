...
internal class Dialog_SetName : WindowPlus
{
    ...
    private Dialog_SetName(string title, string label, Action<string> onCommit, Func<string, bool> validator,
        string name = "") : base(title, new Vector2(400f, 170f))
    {
        doCloseButton = false;
    ...
    public static void Open(string title, string label, Action<string> onCommit, Func<string, bool> validator,
        string name = null)
    {
        Find.WindowStack.Add(new Dialog_SetName(title, label, onCommit, validator, name));
    }
    ...
    {
        return _validator == null || _validator(_name);
    }
