...
internal class Listing_Preset<T> where T : Presetable
{
    ...
    public void DoContent(Rect rect)
    {
        ...
        foreach (var preset in FixedPresets)
        {
            ...
            if (_listing.RadioButtonInverted(preset.GetDisplayName().Italic(), isSelected, null,
                    !EditMode || isSelected))
            {
        ...
        {
            if (_listing.ButtonText(Lang.Get("Button.PresetSaveAs"), Lang.Get("Button.PresetSaveAsDesc"),
                    !Selected.IsVoid && !selectedIsIgnored))
            {
                OnSave?.Invoke();
            }
        }
        ...
        if (_listing.ButtonText(Lang.Get("Button.PresetDelete"), Lang.Get("Button.PresetDeleteDesc"),
                !Selected.IsVoid && Selected.IsPreset && !EditMode))
        {
            Dialog_Alert.Open(Lang.Get("Button.PresetDeleteConfirm", Selected.Name), Dialog_Alert.Buttons.YesNo,
                DeletePreset);
        }

        if (EditMode)
        {
            if (_listing.ButtonText(Lang.Get("Button.PresetRevert"), Lang.Get("Button.PresetRevertDesc"),
                    !Selected.IsVoid))
            {
                Revert();
        ...
        {
            if (_listing.ButtonText(Lang.Get("Button.PresetRename"), Lang.Get("Button.PresetRenameDesc"),
                    !Selected.IsVoid && !EditMode))
            {
        ...
        {
            if (_listing.ButtonText(Lang.Get("Button.PresetClear"), Lang.Get("Button.PresetClearDesc"),
                    !Selected.IsVoid && !selectedIsIgnored))
            {
                OnRevert?.Invoke();
            }
    ...
    public void Revert()
    {
        if (!Selected.IsPreset || _lastSelected == null)
        {
            return;
        }
