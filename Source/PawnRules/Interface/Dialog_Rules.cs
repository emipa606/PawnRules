...
internal class Dialog_Rules : WindowPlus
{
    ...
    {
        Find.WindowStack.Add(new Dialog_Rules(pawn, Registry.GetOrNewRules(pawn)));
    }

    public static void OpenDefaults(PawnType type)
    {
        Find.WindowStack.Add(new Dialog_Rules(type));
    }
    ...
    private void ChangeType(PawnType type)
    {
        ...
        {
            _preset.Selected = Registry.GetDefaultRules(type);
        }
    ...
    private void ChangeRestriction(RestrictionType type)
    {
        ...
        list.Add(new FloatMenuOption(Lang.Get("Dialog_Rules.EditRestriction"),
            () => Dialog_Restrictions.Open(type, _template)));
        ...
        if (!_template.GetRestriction(type).IsVoid)
        {
            list.Add(new FloatMenuOption(
                Lang.Get("Dialog_Rules.ClearRestriction", voidPreset.Type.Categorization.ToLower()),
                () => _template.SetRestriction(type, voidPreset)));
        }

        list.AddRange(presets.Select(restriction =>
            new FloatMenuOption(Lang.Get("Dialog_Rules.ChangeRestriction", restriction.Name.Bold()),
                () => _template.SetRestriction(type, restriction))));
    ...
    private void UpdateSelected()
    {
        ...
        {
            Registry.ReplaceDefaultRules(_type, _preset.Selected);
        }

        UpdateTemplate();
    }

    private IEnumerable<Pawn> GetOtherPawnsOfType(bool byKind)
    {
        return _type == null
            ? Find.CurrentMap.mapPawns.AllPawns.Where(pawn =>
                pawn != _pawn && pawn.GetTargetType() == _preset.Type && (!byKind || pawn.kindDef == _pawn.kindDef))
            : Find.CurrentMap.mapPawns.AllPawns.Where(pawn => pawn.GetTargetType() == _type);
    }
    ...
    private string GetPresetNameDefinite()
    {
        if (_preset.Selected == _personalized)
        {
            return Lang.Get("Dialog_Rules.AssignPersonalizedName");
        }

        return _preset.Selected.IsVoid
            ? Lang.Get("Dialog_Rules.AssignVoidName")
            : Lang.Get("Dialog_Rules.AssignSpecificName", _preset.Selected.Name.Bold());
    }
    ...
    private List<FloatMenuOption> GetAssignmentOptions()
    {
        ...
        {
            options.Add(new FloatMenuOption(Lang.Get("Dialog_Rules.AssignAll", _preset.Type.LabelPlural.ToLower()),
                () => AssignAll(false)));
        }
        ...
        if (_type == null && _pawn.RaceProps.Animal && otherPawnsOfType.Any(kind => kind.kindDef == _pawn.kindDef))
        {
            options.Add(new FloatMenuOption(
                Lang.Get("Dialog_Rules.AssignAll", _pawn.kindDef.GetLabelPlural().ToLower()), () => AssignAll(true)));
        }

        options.AddRange(Find.CurrentMap.mapPawns.AllPawns
            .Where(pawn => (_type != null || pawn != _pawn) && pawn.GetTargetType() == _preset.Type).Select(pawn =>
                new FloatMenuOption(Lang.Get("Dialog_Rules.AssignSpecific", pawn.Name.ToString().Italic()),
                    () => AssignSpecific(pawn))));
        if (_type == null && _preset.Selected.IsPreset)
        {
            options.Add(new FloatMenuOption(Lang.Get("Dialog_Rules.AssignDefault", _preset.Type.LabelPlural.ToLower()),
                () => Dialog_Alert.Open(
                    Lang.Get("Dialog_Rules.AssignDefaultConfirm", _preset.Type.LabelPlural.ToLower(),
                        _preset.Selected.Name.Bold()), Dialog_Alert.Buttons.YesNo,
                    () => Registry.SetDefaultRules(_preset.Selected))));
        }
    ...
    {
        return restriction.IsPreset && !restriction.IsVoid ? restriction.Name.Bold() : restriction.Name;
    }
    ...
    protected override void DoContent(Rect rect)
    {
        ...
        Title = _type == null
            ? Lang.Get("Dialog_Rules.Title", _pawn.Name.ToStringFull.Bold(), _preset.Type.Label)
            : Lang.Get("Dialog_Rules.TitleDefault", _type.LabelPlural.Bold());
        ...
        if (Registry.ShowFoodPolicy &&
            listing.ButtonText(
                Lang.Get("Rules.FoodRestrictions",
                    GetRestrictionDisplayName(_template.GetRestriction(RestrictionType.Food))),
                Lang.Get("Rules.FoodRestrictionsDesc")) && editMode)
        {
        ...
        if (_template.Type == PawnType.Colonist)
        {
            if (Registry.ShowBondingPolicy &&
                listing.ButtonText(
                    Lang.Get("Rules.BondingRestrictions",
                        GetRestrictionDisplayName(_template.GetRestriction(RestrictionType.Bonding))),
                    Lang.Get("Rules.BondingRestrictionsDesc")) && editMode)
            {
                ChangeRestriction(RestrictionType.Bonding);
            }

            if ((Registry.ShowFoodPolicy || Registry.ShowBondingPolicy) &&
                (Registry.ShowAllowCourting || Registry.ShowAllowArtisan))
            {
                listing.GapLine();
            }
            ...
            {
                listing.CheckboxLabeled(Lang.Get("Rules.AllowCourting"), ref _template.AllowCourting,
                    Lang.Get("Rules.AllowCourtingDesc"), editMode);
            }

            if (Registry.ShowAllowArtisan)
            {
                listing.CheckboxLabeled(Lang.Get("Rules.AllowArtisan"), ref _template.AllowArtisan,
                    Lang.Get("Rules.AllowArtisanDesc"), editMode);
            }
        }

        if (Registry.ShowFoodPolicy || Registry.ShowBondingPolicy || Registry.ShowAllowCourting ||
            Registry.ShowAllowArtisan)
        {
            listing.GapLine();
        }
        ...
        if (listing.ButtonText(Lang.Get("Button.AssignTo"), Lang.Get("Button.AssignToDesc"),
                _floatMenuAssign.Count > 0 && (!editMode || _template == _personalized)))
        {
        ...
        if (listing.ButtonText(
                _type == null
                    ? Lang.Get("Button.ViewType", Lang.Get("PawnType.Individual"))
                    : Lang.Get("Button.ViewTypeDefault", _type.LabelPlural), Lang.Get("Button.ViewTypeDesc"),
                !editMode || _template == _personalized))
        {
        ...
        if (GuiPlus.ButtonText(
                new Rect(rect.xMax - (80f - Margin), rect.yMax + (Margin * 2), OptionButtonSize, CloseButSize.y),
                Lang.Get("Button.GlobalOptions"), Lang.Get("Button.GlobalOptionsDesc")))
        {
            Dialog_Global.Open();
        }
