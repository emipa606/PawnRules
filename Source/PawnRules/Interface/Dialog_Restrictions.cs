...
internal class Dialog_Restrictions : WindowPlus
{
    ...
    {
        Find.WindowStack.Add(new Dialog_Restrictions(type, rules));
    }
    ...
    {
        return !_presetList.Selected.IsVoid && !_rules.GetRestriction(_presetList.Selected.Type).Matches(_template);
    }
    ...
    {
        Title = _presetList.EditMode || _rules.Pawn == null
            ? Lang.Get("Dialog_Restrictions.TitlePreset", _type.Label, _presetList.Selected.Name.Bold())
            : Lang.Get("Dialog_Restrictions.TitlePawn", _type.Label, _rules.Pawn.Name.ToStringFull.Bold(),
                _rules.Type.Label);
    ...
    private void DoCategories(Rect categoriesRect, Rect membersRect)
    {
        ...
        foreach (var category in _template.Categories)
        {
            ...
            {
                members[category.Label] = category.Members.Where(member =>
                    member.Def is ThingDef def && (!def.IsDrug || Registry.AllowDrugsRestriction) &&
                    (showAll || _rules.Pawn.RaceProps.CanEverEat(def))).ToArray();
            }
