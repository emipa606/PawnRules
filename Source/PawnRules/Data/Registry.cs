...
internal class Registry : WorldObject
{
    ...
    public static bool IsActive =>
        !_isDeactivating && _instance != null && Current.ProgramState == ProgramState.Playing;

    public static bool ShowFoodPolicy
    {
        get => _instance._showFoodPolicy;
        set => _instance._showFoodPolicy = value;
    }

    public static bool ShowBondingPolicy
    {
        get => _instance._showBondingPolicy;
        set => _instance._showBondingPolicy = value;
    }

    public static bool ShowAllowCourting
    {
        get => _instance._showAllowCourting;
        set => _instance._showAllowCourting = value;
    }

    public static bool ShowAllowArtisan
    {
        get => _instance._showAllowArtisan;
        set => _instance._showAllowArtisan = value;
    }

    public static bool AllowDrugsRestriction
    {
        get => _instance._allowDrugsRestriction;
        set => _instance._allowDrugsRestriction = value;
    }

    public static bool AllowEmergencyFood
    {
        get => _instance._allowEmergencyFood;
        set => _instance._allowEmergencyFood = value;
    }

    public static bool AllowTrainingFood
    {
        get => _instance._allowTrainingFood;
        set => _instance._allowTrainingFood = value;
    }

    public static Pawn ExemptedTrainer
    {
        get => _instance._exemptedTrainer;
        set => _instance._exemptedTrainer = value;
    }
    ...
    public static T GetVoidPreset<T>(IPresetableType type) where T : Presetable
    {
        return (T)_instance._voidPresets[typeof(T)][type];
    }
    ...
    public static T GetPreset<T>(IPresetableType type, string name) where T : Presetable
    {
        if (!_instance._presets.ContainsKey(typeof(T)) || !_instance._presets[typeof(T)].ContainsKey(type) ||
            !_instance._presets[typeof(T)][type].ContainsKey(name))
        {
            return null;
        }
    ...
    {
        if (!_instance._presets.ContainsKey(preset.GetType()))
        {
            _instance._presets[preset.GetType()] = new Dictionary<IPresetableType, Dictionary<string, Presetable>>();
        }

        if (!_instance._presets[preset.GetType()].ContainsKey(preset.Type))
        {
            _instance._presets[preset.GetType()][preset.Type] = new Dictionary<string, Presetable>();
        }
    ...
    {
        if (preset == null || preset.IsVoid)
        {
            throw new Mod.Exception("Tried to delete void preset");
        }

        if (!_instance._presets.ContainsKey(preset.GetType()) ||
            !_instance._presets[preset.GetType()].ContainsKey(preset.Type))
        {
            return;
        }
        ...
        {
            foreach (var rule in _instance._rules.Where(rule => rule.Value == preset).ToArray())
            {
                _instance._rules[rule.Key] = GetVoidPreset<Rules>(rule.Value.Type).CloneRulesFor(rule.Key);
            }

            foreach (var rule in _instance._defaults.Where(rule => rule.Value == preset).ToArray())
            {
                _instance._defaults[rule.Key] = GetVoidPreset<Rules>(rule.Value.Type);
            }
        }
        else if (typeof(T) == typeof(Restriction) && _instance._presets.ContainsKey(typeof(Rules)))
        {
            foreach (var rulesType in _instance._presets[typeof(Rules)].Values.ToArray())
            {
                foreach (var presetable in rulesType.Values.ToArray())
                {
                    ...
                    if (rule.GetRestriction(type) == preset)
                    {
                        rule.SetRestriction(type, GetVoidPreset<Restriction>(type));
                    }
                }
            }
        }
    ...
    public static bool PresetNameExists<T>(IPresetableType type, string name)
    {
        return _instance._presets.ContainsKey(typeof(T)) && _instance._presets[typeof(T)].ContainsKey(type) &&
               _instance._presets[typeof(T)][type].ContainsKey(name);
    }
    ...
    {
        _instance._defaults[rules.Type] = rules;
    }
    ...
    public static Rules GetOrNewRules(Pawn pawn)
    {
        ...
        if (_instance._rules.TryGetValue(pawn, out var newRules))
        {
            return newRules;
        }
    ...
    {
        _instance._rules[pawn] = rules;
    }

    public static void ReplaceDefaultRules(PawnType type, Rules rules)
    {
        _instance._defaults[type] = rules;
    }
    ...
    {
        if (!IsActive || thing is not Pawn pawn || pawn.Dead)
        {
            return;
        }

        Faction oldFaction = null;
        if (pawnType == PawnType.Guest)
        {
            oldFaction = pawn.Faction;
        }

        if (pawnType == PawnType.Slave || pawnType == PawnType.Prisoner)
        {
            oldFaction = pawn.HostFaction;
        }

        if (newFaction is { IsPlayer: true })
        {
            ChangeTypeOrCreateRules(pawn, pawnType);
            return;
        }

        if (oldFaction is { IsPlayer: true })
        {
            DeleteRules(pawn);
        }
    }
    ...
    public static void DeactivateMod()
    {
        ...
        if (Find.WorldObjects.Contains(_instance))
        {
            Find.WorldObjects.Remove(_instance);
        }
        ...
        var message = addonMods.Length > 0
            ? Lang.Get("Button.RemoveModAndAddonsComplete", saveName.Bold(), addonMods.ToString())
            : Lang.Get("Button.RemoveModComplete", saveName.Bold());
    ...
    private void InitVoids()
    {
        ...
        {
            _voidPresets[typeof(Rules)][type] = Presetable.CreateVoidPreset<Rules>(type);
        }

        _voidPresets[typeof(Restriction)] = new Dictionary<IPresetableType, Presetable>();
        foreach (var type in RestrictionType.List)
        {
            _voidPresets[typeof(Restriction)][type] = Presetable.CreateVoidPreset<Restriction>(type);
        }
    }

    private void InitDefaults()
    {
        foreach (var type in PawnType.List.Where(type => !_defaults.ContainsKey(type)))
        {
            _defaults.Add(type, (Rules)_voidPresets[typeof(Rules)][type]);
        }
    }
    ...
    public override void ExposeData()
    {
        ...
        if (Scribe.mode == LoadSaveMode.PostLoadInit && _loadedVersion != CurrentVersion)
        {
            Mod.Warning(
                $"Registry loaded was saved by a different mod version ({_loadedVersion ?? "vNULL"} loaded, current is {CurrentVersion})");
        }
        ...
        if (Scribe.mode == LoadSaveMode.Saving)
        {
            ...
            foreach (var type in _presets.Values.SelectMany(classType => classType.Values))
            {
                _savedPresets.AddRange(type.Values.ToArray());
            }

            _savedDefaults.AddRange(_defaults.Values.Where(preset => !preset.IsVoid)
                .Select(preset => new Binding(preset.Type, preset)).ToArray());
            _savedBindings.AddRange(_rules.Where(rules => rules.Key.CanHaveRules())
                .Select(rules => new Binding(rules.Key, rules.Value.IsIgnored() ? null : rules.Value)).ToArray());
        }
    ...
    public static XElement ToXml()
    {
        ...
        {
            foreach (var presetType in type.Values)
            {
                presets.Add(from preset in presetType where !preset.Value.IsVoid select preset.Value.ToXml());
            }
        }

        xml.Add(presets);

        xml.Add(new XElement("Defaults",
            from rule in _instance._defaults.Values
            where !rule.IsVoid
            select new XElement("Preset", new XAttribute("Type", rule.Type.Id), rule.Name)));
    ...
    public static void FromXml(XElement xml)
    {
        ...
        {
            Mod.Warning(
                $"Loaded xml from a different mod version ({version ?? "vNULL"} loaded, current is {CurrentVersion})");
        }
        ...
        foreach (var restriction in presets.Where(preset => preset.Name == "Restriction")
                     .Select(preset => new Restriction(preset)))
        {
            AddPreset(restriction);
        }

        foreach (var rules in presets.Where(preset => preset.Name == "Rules").Select(preset => new Rules(preset)))
        {
            AddPreset(rules);
        }
