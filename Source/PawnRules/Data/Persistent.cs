...
internal static class Persistent
{
    ...
    private static FileInfo GetPlanFile(string name)
    {
        return new FileInfo(Path.Combine(ExportsDirectory.FullName, ExportPrefix + name + ExportsExtension));
    }
    ...
    {
        return !name.NullOrEmpty() && name.Length <= 250 - ExportsDirectory.FullName.Length &&
